...
import { doc, setDoc, getDoc, collection, addDoc, updateDoc, getDocs, deleteDoc } from 'firebase/firestore';
...
export function CartTestComponent() {
  ...
  const testDatabaseWrite = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        setTestResult(`Success! Test data written to: users/${userId}/test/test-doc\nData: ${JSON.stringify(docSnap.data(), null, 2)}`);
      } else {
        setTestResult(`Warning: Write completed but data not found at: users/${userId}/test/test-doc`);
      }
    } catch (error) {
  ...
  const testAddToCart = async () => {
    ...
    try {
      ...
      const productDocRef = doc(db, 'users', userId, 'carts', testBusinessId, 'products', testProductId);
      ...
      if (productSnap.exists()) {
        ...
        setTestResult(`Success! Test product added to cart at: users/${userId}/carts/${testBusinessId}/products/${testProductId}\nData: ${JSON.stringify(productSnap.data(), null, 2)}`);
      } else {
        setTestResult(`Warning: Write completed but product not found at: users/${userId}/carts/${testBusinessId}/products/${testProductId}`);
      }
    } catch (error) {
  ...
  const readAllCartData = async () => {
    ...
    try {
      ...
      setTestResult(`Cart data found for user ${userId}:\n${JSON.stringify(cartData, null, 2)}`);
    } catch (error) {
  ...
  const listCartProducts = async () => {
    ...
    try {
        ...
        if (!productsSnapshot.empty) {
          productsSnapshot.forEach(productDoc => {
            ...
            productList.push({
              ...
              totalPrice: (productData.price || 0) * (productData.quantity || 0)
            });
          });
        }
      }
      ...
      if (productList.length === 0) {
        ...
        const totalValue = productList.reduce((sum, product) => sum + product.totalPrice, 0);
        setTestResult(
  ...
  const checkSecurityRules = async () => {
    ...
    try {
      ...
      const paths = [
        ...
        { name: 'User test subcollection', path: ['users', userId, 'test', 'security-test'] },
        { name: 'User carts collection', path: ['users', userId, 'carts', 'security-test'] },
        { name: 'User cart products', path: ['users', userId, 'carts', 'security-test', 'products', 'test-product'] }
      ];
      ...
      for (const pathInfo of paths) {
        try {
          ...
          results[pathInfo.name] = `Write successful to ${pathInfo.path.join('/')}`;
        } catch (error) {
          results[pathInfo.name] = `Write failed to ${pathInfo.path.join('/')}: ${error.message}`;
        }
      }
      
      setTestResult(`Security rules check results:\n${JSON.stringify(results, null, 2)}`);
    } catch (error) {
  ...
  const testCreateCollection = async () => {
    ...
    try {
      ...
      if (collectionSnap.exists() && item1Snap.exists() && item2Snap.exists()) {
        ...
        setTestResult(`Success! Test collection created at: cart_collections/${userId}\nMain Data: ${JSON.stringify(collectionSnap.data(), null, 2)}\nItem 1: ${JSON.stringify(item1Snap.data(), null, 2)}\nItem 2: ${JSON.stringify(item2Snap.data(), null, 2)}`);
      } else {
        setTestResult(`Warning: Write completed but not all data found at: cart_collections/${userId}`);
      }
    } catch (error) {
  ...
  const clearEntireCart = async () => {
    ...
    try {
      ...
      for (const businessDoc of cartsSnapshot.docs) {
        ...
        const productDeletePromises = productsSnapshot.docs.map(async productDoc => {
          await deleteDoc(doc(db, 'users', userId, 'carts', businessId, 'products', productDoc.id));
          deletedProducts++;
        });
  ...
  return (
    <div className="p-6 border rounded-lg mb-6">
      ...
      {userId ? (
        ...
        <p className="mb-4 text-red-500">Not logged in. Please log in to test cart functionality.</p>
      )}
