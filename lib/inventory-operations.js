...
import { ref as databaseRef, push, set, get, update, remove, onValue } from "firebase/database"
...
export const uploadProductImageToDatabase = async (businessId, file) => {
  try {
    const fileRef = storageRef(storage, `products/${businessId}/${Date.now()}_${file.name}`);
...
// Record a purchase
export const recordPurchaseFromDatabase = async (userId, productId, quantity, price) => {
  try {
