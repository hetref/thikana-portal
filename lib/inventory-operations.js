...
export async function getUserAnalytics(userId) {
  try {
    ...
    if (userAnalyticsDoc.exists()) {
      console.log("USERDATA", userAnalyticsDoc.data())
...
export async function getMonthlyAnalytics(userId, limitValue = 12) {
  try {
    const monthlyAnalyticsRef = collection(db, `analytics/${userId}/monthly`)
    const querySnapshot = await getDocs(query(monthlyAnalyticsRef, orderBy("month", "desc"), limit(limitValue)))
    console.log("MONTHLY QUERY SNAPSHOT", querySnapshot)
    querySnapshot.docs.forEach((doc) => console.log("SNAPPING:", doc.id, doc.data()))
...
export async function getYearlyAnalytics(userId, limitValue = 5) {
  try {
    const yearlyAnalyticsRef = collection(db, `analytics/${userId}/yearly`)
    const querySnapshot = await getDocs(query(yearlyAnalyticsRef, orderBy("year", "desc"), limit(limitValue)))
    return querySnapshot.docs
      .map(
        (doc) =>
        ({
