...
export async function recordPurchase(userId, productId, quantity, price) {
  try {
    await runTransaction(db, async (transaction) => {
      ...
      if (!userAnalyticsDoc.data().totalSales) {
        ...
        transaction.update(userAnalyticsRef, {
          ...
          totalRevenue: increment(price * quantity),
          averageOrderValue: Number(newTotalRevenue) / Number(newTotalSales),
        })
        console.log("User analytics updated successfully", Number(newTotalRevenue) / Number(newTotalSales))
      }
    })
  } catch (error) {
