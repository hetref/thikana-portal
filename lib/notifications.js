    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const notifications = snapshot.docs.map((doc) => {
          ...
          let formattedTs = "Just now";

          try {
            if (rawTs?.toDate) {
              // Firestore Timestamp
              formattedTs = rawTs.toDate().toLocaleString();
            } else if (rawTs instanceof Date) {
              formattedTs = rawTs.toLocaleString();
            } else if (typeof rawTs === "number") {
              // Unix ms
              formattedTs = new Date(rawTs).toLocaleString();
            } else if (typeof rawTs === "string") {
              formattedTs = rawTs;
            }
          } catch (e) {
            console.error("Error formatting notification timestamp for doc:", doc.id, e);
          }
