...
export async function POST(req) {
  try {
    ...
    console.log(`Processing failed calls for business ${businessId}`);

    // Find all calls that need manual processing or failed to process
    const requestsRef = collection(db, "users", businessId, "requestCalls");
    const q = query(requestsRef, where("needsManualProcessing", "==", true));
    const querySnapshot = await getDocs(q);

    if (querySnapshot.empty) {
      return NextResponse.json({
        success: true,
        message: "No failed calls found to retry",
        processedCount: 0,
      });
    }

    const failedCalls = querySnapshot.docs.map((doc) => ({
      id: doc.id,
      ref: doc.ref,
      ...doc.data(),
    }));

    console.log(`Found ${failedCalls.length} calls to retry processing`);

    let successCount = 0;
    let failCount = 0;
    const results = [];

    // Process each failed call
    for (const call of failedCalls) {
      if (!call.callId) {
        console.log(`Skipping call ${call.id} - no callId`);
        continue;
      }

      try {
        console.log(`Retrying processing for call ${call.callId}`);

        // Wait before fetching call details as VAPI might need time to process
        console.log(
          `Waiting 3 seconds before fetching call details for ${call.callId}...`
        );
        await new Promise((resolve) => setTimeout(resolve, 3000)); // Wait 3 seconds

        // Fetch call details with retry logic
        const callDetails = await fetchCallDetailsWithRetry(call.callId, 5);
        ...
        if (callDetails) {
          ...
          if (callDetails.status === "ended" && call.status === "initiated") {
            updateData.status =
          ...
          await updateDoc(call.ref, updateData);
        ...
        await updateDoc(call.ref, {
          callDetailsError: error.message,
          lastRetryAt: serverTimestamp(),
    ...
    console.log(
      `Retry processing complete: ${successCount} success, ${failCount} failed`
    );

    return NextResponse.json({
      success: true,
      message: `Processed ${successCount + failCount} calls`,
