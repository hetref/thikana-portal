...
export async function GET(req, { params }) {
  const { userId } = await params; // Extract userId from URL parameters
...
export async function POST(req, { params }) {
  const { userId } = params; // Extract userId from URL parameters
  console.log('userId:', userId);

  const { razorpayKeyId, razorpayKeySecret } = await req.json(); // Parse JSON body
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: 'Payment settings updated successfully' }), { status: 200 });
  } catch (error) {
